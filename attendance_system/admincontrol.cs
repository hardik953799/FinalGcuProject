    ...
    public partial class admincontrol1 : UserControl
    {
        ...
        private void panel_UpPaswd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (passbox.Text == "")
                {
                    query = "update facerecognition.teacher set rank = '" + rank + "', faculty = '" + faculty + "', department = '" + dept + "', email = '" + email + "' where id = " + Form1.user_id + "  ";

                }
                else
                {
                    query = "update facerecognition.teacher set rank = '" + rank + "', faculty = '" + faculty + "', department = '" + dept + "', email = '" + email + "',   password = '" + pass + "' where id = " + Form1.user_id + "  ";
        ...
        private void admincontrol1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = " select * from facerecognition.teacher where id = " + Form1.user_id + "  ";
                MySqlConnection conn2 = new MySqlConnection(conn);
