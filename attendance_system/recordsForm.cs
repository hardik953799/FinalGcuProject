    ...
    public partial class recordsForm : Form
    {
        ...
        private void show_data()
        {
              ...
              try
              {
                  ...
                  string query = "select distinct `date` from facerecognition.attendance where `course_id` = " + classControl.course_id + " ";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 2)  //delete
            {
                ...
                {
                    MySqlConnection conn2 = new MySqlConnection(conn);
                    string query = " delete from facerecognition.attendance where course_id = " + classControl.course_id + " and `date` = '" + dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString() + "' ";
        ...
        private int checkStudent()
        {
            ...
            MySqlConnection conn2 = new MySqlConnection(conn);
            string query = "select * from facerecognition.student where  course_id = " + classControl.course_id + "  ";
            //  MessageBox.Show(query);
            MySqlDataAdapter sda = new MySqlDataAdapter(query, conn2);
            DataTable dtbt = new DataTable();
            sda.Fill(dtbt);
        ...
        {
            trainingSetManage addFacerecognize = new trainingSetManage();
            addFacerecognize.StartPosition = FormStartPosition.CenterParent;
            ((Form)this.TopLevelControl).Hide();

            addFacerecognize.ShowDialog();
            ((Form)this.TopLevelControl).Show();
        }
    }
}
