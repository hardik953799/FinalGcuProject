...
namespace attendance_system
{
    public partial class TakeAttendanceForm : Form
    {
        ...
        private void show_data()
        {
            try
            {
                string query = "select * from facerecognition.student where `course_id` = '" + classControl.course_id + "' order by roll ";
        ...
        private void buttonSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    string query = "insert into facerecognition.attendance(student_id, student_roll, course_code, course_id, attendance_value, date) values('" + row.Cells[6].Value + "', " + row.Cells[0].Value + ", '" + classControl.course_code + "' , " + classControl.course_id + " ,'" + attendance_value + "', '" + DateTime.Now.ToString("dd-MM-yy") + "') ";
                  //  MessageBox.Show(query);
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 5)  //delete
            {
                ...
                {
                    MySqlConnection conn2 = new MySqlConnection(conn);
                    string query = " delete from facerecognition.student where id = " + Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[6].Value.ToString()) + " and course_id = " + classControl.course_id + " ";
                   // MessageBox.Show(query);
