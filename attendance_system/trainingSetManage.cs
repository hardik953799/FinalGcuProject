        ...
        public void ConnectToDatabase()
        {
          ...
          //access//  dataAdapter = new OleDbDataAdapter("select * from trainingSet1", dbConnection);
            //access//  OleDbCommandBuilder commandBuilder = new OleDbCommandBuilder(dataAdapter);
             dataAdapter = new MySqlDataAdapter("select * from facerecognition.trainingSet1 where course_id = "+classControl.course_id+"", dbConnection);
        ...
        public void addFaceToDB(Image inputFace, string faceName, int userID, string session)
        {
            ...
            try
            {
               ...
               // MessageBox.Show("Adding row " + totalRows);
                ...
                MySqlCommand insertCommand = new MySqlCommand("Insert into facerecognition.trainingSet1(roll,name, faceImage, course_code, course_id, session) Values('" + userID + "','" + faceName + "', @img, '"+ classControl.course_code +"', "+ classControl.course_id +", '"+session+"')", dbConnection);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    ...
                    string query = " delete from facerecognition.trainingset1 where roll = " + textBoxUserIdTrainingSet.Text + " and course_id = " + classControl.course_id + " and name = '" + textBoxNameTraingSetViewer.Text + "' ";
