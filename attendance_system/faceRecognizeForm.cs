...
namespace attendance_system
{
    public partial class faceRecognizeForm : Form
    {
       ...
       // OleDbDataAdapter dataAdapter;
        ...
        string conn = "Server=localhost;Port=3306;Database=FaceRecognition;Uid=root;Pwd=H@rdik480";
        ...
        private void show_data()
        {
            try
            {
                string query = "select  * from facerecognition.trainingset1  where `course_id` = " + classControl.course_id + " group by roll order by roll ";
        ...
        public void ConnectToDatabase(int userid = -1)
        {
            ...
            if (userid == -1)
            {
                ...
                dataAdapter = new MySqlDataAdapter("select * from facerecognition.trainingSet1", dbConnection);
            }
            else
            {

                dataAdapter = new MySqlDataAdapter("select * from facerecognition.trainingSet1 where roll = " + userid + " ", dbConnection);
            }
        ...
        private int checkDate()
        {
            ...
            MySqlConnection conn2 = new MySqlConnection(conn);
            string query = "select distinct `date` from facerecognition.attendance where  course_id = " + classControl.course_id + "  and date = '" + DateTime.Now.ToString("dd-MM-yy") + "' ";
        ...
        private void buttonSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    string query = "insert into facerecognition.attendance(student_id, student_roll, course_code, course_id, attendance_value, date) values('" + row.Cells[4].Value + "', " + row.Cells[0].Value + ", '" + classControl.course_code + "' , " + classControl.course_id + " ,'" + attendance_value + "', '" + DateTime.Now.ToString("dd-MM-yy") + "') ";
                   // MessageBox.Show(query);
