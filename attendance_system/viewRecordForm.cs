...
namespace attendance_system
{
    public partial class viewRecordForm : Form
    {
         ...
         private void show_data()
        {
            try
            {
                string  query = "select std.id, std.roll, std.name, std.course_id, att.student_id, att.attendance_value,att.date,att.id  from facerecognition.student std inner join facerecognition.attendance att on std.course_id = att.course_id  and att.course_id = "+classControl.course_id+" and att.date = '"+recordsForm.clicked_date+"' and std.roll = att.student_roll order by std.roll";
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    string query = "UPDATE facerecognition.attendance SET `attendance_value`='" + attendance_value + "' WHERE `course_id` = " + classControl.course_id + " AND `date` = '" + recordsForm.clicked_date + "' AND `id` = " + Convert.ToInt32(row.Cells[5].Value.ToString()) + "  ";
                  //  MessageBox.Show(query);
